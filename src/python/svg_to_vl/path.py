...
class Path:
    ...
    def splitValues(self):
        ...
        currentCommand = ''
        for v in self.line:
            if v.isnumeric() == lastChar.isnumeric() or v == '.' or  lastChar == '.' or lastChar == '-' or lastChar == '+' :
                currentCommand  += v
                lastChar = v
            else:
                if (not currentCommand.isspace()) and (currentCommand != '') and (currentCommand != ','):
                    self.commandElements.append(currentCommand.strip())
                if v != ',':
                    currentCommand=v
                    lastChar = v
                else:
