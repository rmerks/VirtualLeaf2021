...
class Sprinzak : public QObject, SimPluginInterface {
	Q_OBJECT
    Q_INTERFACES(SimPluginInterface)
        Q_PLUGIN_METADATA(IID "org.virtualleaf.Sprinzak")
...
public:
    ...
    virtual double NOTCHflux(CellBase *this_cell, CellBase *adjacent_cell, Wall *w);
    virtual double DELTAflux(CellBase *this_cell, CellBase *adjacent_cell, Wall *w);

    // default XML file to be loaded on startup
    virtual QString DefaultLeafML(void) { return QString("hexagonalgrid.xml"); }
};
